...
export default function PostDetailClient({ initialPost, postId }: Props) {
  ...
  const [post, setPost] = useState<AuraWithUser | null>(initialPost);
  ...
  // ── Secondary data fetches (city, token, user profile, place) ───────────────
  useEffect(() => {
    if (!post) return;
    ...
    Promise.allSettled([
      ...
      post.lat && post.lng
        ? Promise.all([
            getCityFromCoordinates(post.lat, post.lng),
            fetch("/api/mapbox-token").then((r) => r.json()),
          ])
        : Promise.resolve(null),
      // 1: user display name (only if not embedded)
      !post.user?.name && !post.user_name && post.user_id
        ? fetch(`${API_BASE}/api/users/${post.user_id}`, { headers: authHeader })
            .then((r) => r.ok ? r.json() : null)
        : Promise.resolve(null),
      // 2: place data — graceful 404 if backend not yet ready
      post.place_id
        ? fetch(`${API_BASE}/api/places/${post.place_id}`, { headers: authHeader })
            .then((r) => r.ok ? r.json() : null)
        : Promise.resolve(null),
    ]).then(([cityTokenResult, userProfileResult, placeResult]) => {
      ...
      if (placeResult.status === "fulfilled" && placeResult.value) {
        ...
        if (pd.place) {
          setPlace(pd.place);
          setPlacePosts((pd.posts ?? []).filter((p) => p.id !== post.id));
        }
      }
    });
