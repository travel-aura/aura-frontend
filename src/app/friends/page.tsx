    ...
    <div className="relative flex min-h-screen w-full flex-col bg-[#F7F3EC]">
      ...
      <div className="px-4 pt-4">
        <div className="flex items-center gap-2 rounded-xl bg-[#EDE6D9] px-3 py-3">
          <svg className="size-4 shrink-0 text-[#6B5F52]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
          ...
          {query.length > 0 && (
            <button onClick={() => setQuery("")}>
              <svg className="size-4 text-[#6B5F52]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round">
      ...
      <div className="mt-2 flex-1 overflow-y-auto">
        ...
        {!loading && query.length < 2 && (
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            <svg className="size-12 text-[#D4C4A8]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
            ...
            {results.map((user) => (
              <li key={user.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <Link href={`/profile/${user.id}`} className="min-w-0 flex-1">
                  <p className="truncate text-[15px] font-semibold text-[#1A1613]">
                    {displayName(user)}
                  </p>
                  {user.name && (
